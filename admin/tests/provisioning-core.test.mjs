...
test("apply creates new auth once, repairs existing profile, and writes direct class links", async () => {
    ...
    assert.equal(adapter.writes.profiles.length, 0, "unchanged rerun performs zero Firestore writes");
    assert.equal(second.writes, 0, "unchanged rerun reports zero writes");
    assert(second.results.every(item => item.status === "UNCHANGED"));
});
...
test("missing teacher code fails before Firebase access", async () => {
    const adapter = new Proxy({}, { get() { throw new Error("Firebase should not be accessed"); } });
